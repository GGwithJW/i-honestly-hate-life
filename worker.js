...
function corsHeaders() {
  return {
    ...
    'Access-Control-Expose-Headers': 'Content-Length, Content-Range, Content-Type, Date, Server, X-Cache-Hit, X-Upstream-Status',
...
function parseCustomHeaders(searchParams) {
  ...
  if (headersParam) {
    try {
      let obj;
      try { obj = JSON.parse(headersParam); }
      catch { obj = JSON.parse(atob(headersParam)); }
...
async function handleFetch(request) {
  ...
  if (!upstreamResponse.ok && upstreamResponse.status !== 206) {
    return new Response(
      JSON.stringify({ error: 'Failed to fetch resource', status: upstreamResponse.status }),
...
async function handleForward(request, isM3U8Route) {
  const url         = new URL(request.url);
...
export default {
  async fetch(request) {
    ...
    if (FORWARDED_PATHS.has(url.pathname)) {
      ...
      return handleForward(request, isM3U8Route);
    }
